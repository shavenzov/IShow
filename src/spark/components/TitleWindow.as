...
import com.controls.LinkButton;
...
public class TitleWindow extends Panel
{
	...
	public var _icon : mx.controls.Image;
    ...
    override protected function partAdded(partName:String, instance:Object) : void
    {
		...
		else if ( instance == _icon )
		{
			_icon.source = getStyle("icon");
		}
    }
    ...
    protected function moveArea_mouseDownHandler(event:MouseEvent):void
    {
        ...
        if (enabled /*modified*/ /*&& isPopUp*/ /*modified*/)
        {
	...
	/*
	Компонент ограничиваюший перемещение окна
	*/
	
	private var _parentBorder : DisplayObject;
	
	public function get parentBorder() : DisplayObject
	{
		return _parentBorder ? _parentBorder : parent;
	}
	
	public function set parentBorder( value : DisplayObject ) : void
	{
		_parentBorder = value;
	}
	
	private function correctBounds( rect : Rectangle ) : Rectangle
	{
		if ( rect.x < 0.0 )
		{
			rect.x = 0.0;
		...
		if ( rect.right > parentBorder.width )
		{
			rect.x = parentBorder.width - rect.width;
		}
		
		if ( rect.bottom > parentBorder.height )
		{
			rect.y = parentBorder.height - rect.height;
		}
	...
	/**
	 * Стартовое положение контрола ( right, bottom ) 
	 */	 
	public var initialPos : String;
	
	override protected function createChildren() : void
	{
		super.createChildren();
		
		if ( parentBorder is UIComponent )
		{
			if ( UIComponent( parentBorder ).initialized )
			{
				setInitialPos();
			}
			else
			{
				parentBorder.addEventListener( FlexEvent.CREATION_COMPLETE, onParentBorderCreationComplete );	
			}
		}
		else
		{
	...
	private function onParentBorderCreationComplete( e : FlexEvent ) : void
	{
		parentBorder.removeEventListener( FlexEvent.CREATION_COMPLETE, onParentBorderCreationComplete );
		setInitialPos();
	}
	
	private function setInitialPos() : void
	{
		if ( ! initialPos || ! visible )
		{
			return;
		}
		...
		if ( initialPos == 'right' )
		{
			_x = parentBorder.width - width;
		}
		
		if ( initialPos == 'bottom' )
		{
			_y = parentBorder.height - height;
		}
    ...
    protected function moveArea_mouseMoveHandler( event : MouseEvent ) : void
    {
		...
		//Ограничиваем область перетаскивания родительским компонентом
		correctBounds( afterBounds );
