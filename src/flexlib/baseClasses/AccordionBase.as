...
package flexlib.baseClasses
{
  ...
  public class AccordionBase extends Container implements IHistoryManagerClient, IFocusManagerComponent
  {
    ...
    override public function get baselinePosition():Number
    {
      ...
      if (isEmpty)
      {
        var child0 : NavigatorContent = new NavigatorContent();
        addChild(child0);
        validateNow();
      }
    ...
    override mx_internal function setActualCreationPolicies(policy:String):void
    {
      ...
      if (policy == ContainerCreationPolicy.ALL && numChildren > 0)
      {
        ...
        for (var i:int = 0; i < n; i++)
        {
          INavigatorContent(getChildAt(i)).createDeferredContent();
        }
      }
    }
    ...
    private function commitSelectedIndex():void
    {
      ...
      if (getStyle("openDuration") == 0 || oldIndex == -1)
      {
        ...
        UIComponent(getChildAt(newIndex)).setVisible(true);

        // Now that the effects have been triggered, we can hide the
        // current view until it is properly sized and positioned below.
		UIComponent(getChildAt(newIndex)).setVisible(false, true);
        if (oldIndex != -1)
			INavigatorContent(getChildAt(oldIndex)).setVisible(false);
     ...
     *  @private
     */
    protected function instantiateChild( child : INavigatorContent ):void
    {
      // fix for bug#137430
    ...
    protected function startTween(oldSelectedIndex:int, newSelectedIndex:int):void
    {
      ...
      if (oldSelectedIndex != -1)
		  INavigatorContent(getChildAt(oldSelectedIndex)).tweeningProperties = ["x", "y", "width", "height"];
	  INavigatorContent(getChildAt(newSelectedIndex)).tweeningProperties = ["x", "y", "width", "height"];
      ...
      var newSelectedChild:INavigatorContent = INavigatorContent(getChildAt(newSelectedIndex));
      if ( UIComponent( newSelectedChild ).numChildren == 0)
      {
        var paddingLeft:Number = getStyle("paddingLeft");
    ...
    mx_internal function onTweenUpdate(value:Number):void
    {
      ...
      {
        var header:Button = getHeaderAt(i);
        var content:INavigatorContent = INavigatorContent(getChildAt(i));
    ...
    mx_internal function onTweenEnd(value:Number):void
    {
      ...
      for (var i:int = 0; i < n; i++)
      {
        ...
        if (i == selectedIndex)
        {
          content = INavigatorContent(getChildAt(i));
      ...
      if (oldSelectedIndex != -1)
      {
        content = INavigatorContent(getChildAt(oldSelectedIndex));
        content.cacheAsBitmap = false;
        content.scrollRect = null;
        content.visible = false;
	  ...
	  INavigatorContent(getChildAt(selectedIndex)).tweeningProperties = null;
